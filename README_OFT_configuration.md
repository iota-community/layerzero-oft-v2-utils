...
- [Pathway from IOTA EVM to IOTA L1](./.env.IOTAEVM_IOTAL1)
...
- `lzEndpointOnCurrentChain`: LZ endpoint contract address on EVM as src chain.
- `lzEndpointIdOnCurrentChain`: LZ endpoint ID on EVM as src chain.
- `lzEndpointIdOnRemoteChain`: LZ endpoint ID on as dest chain.
...
- `executorGasDropInWeiOnDestChain`: set to zero
